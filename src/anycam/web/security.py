"""Security middleware for AnyCam.

The trust model is "the network is the boundary" — AnyCam binds to localhost
...
2. A cross-origin guard on state-changing requests so a malicious web page the
   user happens to visit can't drive their local/tailnet AnyCam (CSRF / drive-by
