...
REF="${TAILCAM_REF:-}"
VERSION="${TAILCAM_VERSION:-1.10.0}"
...
can_sudo() {
    ...
    if sudo -n true 2>/dev/null; then
        SUDO_CHECKED=0
    elif [ "$NONINTERACTIVE" = 0 ] && [ -c /dev/tty ] && sudo -v </dev/tty; then
        SUDO_CHECKED=0
    else
        SUDO_CHECKED=1
    fi
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        -h|--help)
            ...
            echo "Release: --version X.Y.Z (PyPI, default 1.10.0); --ref REF (explicit source override)"
...
ensure_system_deps() {
    ...
    local optional=""
    case "$PRESET" in camera|all-in-one) optional="ffmpeg v4l-utils" ;; esac
    if ! can_sudo; then
    ...
    if [ -n "$optional" ]; then sudo apt-get install -y $optional || true; fi
}
...
tune_raspberry_pi() {
    node_has_role capture || return 0
