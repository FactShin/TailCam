#!/usr/bin/env bash
# TailCam uninstaller for macOS. Also cleans up pre-rename AnyCam installs.
set -eu

VENV_DIR="${HOME}/.local/share/tailcam/venv"
LEGACY_VENV_DIR="${HOME}/.local/share/anycam/venv"
DATA_DIRS=""
for d in "${TAILCAM_DATA_DIR:-}" "${ANYCAM_DATA_DIR:-}" "${HOME}/Library/Application Support/TailCam" "${HOME}/Library/Application Support/AnyCam"; do
